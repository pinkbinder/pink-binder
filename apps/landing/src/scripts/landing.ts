...
function renderListing(
  ...
  const image = listing.imageUrl
    ...
    : '<div class="from-primary/20 to-secondary bg-linear-to-br flex h-full w-full items-center justify-center"><span class="text-muted-foreground text-xs select-none">No image</span></div>'
  const logo =
    marketplaceName && marketplaceLogoUrl
      ? `<span class="absolute top-1.5 right-1.5 z-10 flex h-7 w-7 items-center justify-center overflow-hidden rounded-full border border-brand-light-pink/80 bg-white p-1 shadow-xs" title="${escapeHtml(marketplaceName)}"><img src="${escapeHtml(marketplaceLogoUrl)}" alt="${escapeHtml(marketplaceName)} logo" class="h-full w-full object-contain" loading="lazy" decoding="async" /></span>`
      : ''

  return `<a href="${escapeHtml(listing.listingUrl)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHtml(ariaLabel)}" data-listing-id="${escapeHtml(listing.id)}" data-listing-title="${escapeHtml(listing.title)}" data-listing-category="${escapeHtml(listing.source ?? marketplaceName)}" data-listing-currency="${escapeHtml(listing.currency || 'USD')}" class="group flex w-36 shrink-0 flex-col overflow-hidden rounded-2xl border border-brand-light-pink/70 bg-white shadow-xs transition-colors hover:bg-secondary/50"><div class="bg-muted relative flex w-full items-center justify-center overflow-hidden" style="aspect-ratio: ${escapeHtml(imageAspectRatio)}">${logo}${image}</div><div class="flex flex-col gap-1 p-2.5"><p class="text-foreground line-clamp-2 text-xs leading-tight font-medium">${escapeHtml(listing.title)}</p>${price ? `<p class="text-primary-deep text-sm font-bold">${escapeHtml(price)}</p>` : ''}</div></a>`
}
