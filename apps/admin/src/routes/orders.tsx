...
import { formatAmountCents, useAdminSelection } from '../stores/preferences'
...
function OrdersPage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="font-title text-2xl font-bold tracking-tight">Orders</h1>
        <p className="text-muted-foreground mt-1">Recent orders across every sales channel.</p>
      </div>
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.id} className="border-b last:border-0">
                ...
                <td className="py-3 tabular-nums">{formatAmountCents(order.amountCents)}</td>
                <td className="py-3">
                  <Badge
                    variant={
