...
function MarketplaceListingCard({ listing, className, ...props }: MarketplaceListingCardProps) {
  ...
  return (
    <a
      ...
      aria-label={`${listing.title}${formattedPrice ? ` — ${formattedPrice}` : ''}${sourceLabel} (opens in new tab)`}
      className={cn(
