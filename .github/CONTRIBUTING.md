# Contributing to Pink Binder Frontend Monorepo

Welcome! This document is the **single source of truth** for the contribution workflow across all Pink Binder repositories.  
...
```bash
# Clone
git clone git@github.com:0xPlayerOne/pink-binder.git
cd pink-binder
...
For Pink Binder team members and automation agents.
...
For open-source contributors outside Pink Binder.
...
```bash
git clone git@github.com:YOUR_USERNAME/pink-binder.git
cd pink-binder
```
...
```bash
git remote add upstream git@github.com:0xPlayerOne/pink-binder.git
git fetch upstream
...
_Maintainers: Pink Binder engineering team_
