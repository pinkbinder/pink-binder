...
The Git workflow is `direct`: topic branches merge directly into `main`, and the Release Please version PR lands on `main` using `release_merge_strategy`. Feature branches never touch an integration branch; repositories with a real preview or staging environment must opt into `git_workflow: staging-release` explicitly.
...
10. Squash-merge after required checks pass and the change is ready.
...
| ------------------------- | ------ | ----------------------------------- | ---------------------------------- |
| Working branch            | `main` | Squash                              | All required checks pass           |
| Release Please version PR | `main` | Configured `release_merge_strategy` | Validation and release-policy pass |
